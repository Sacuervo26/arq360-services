        ...
        {deliverables.map((item, index) => (
          <Reveal className={index === 0 ? "deliverable--wide" : ""} delay={(index % 4) * .04} key={item.title}>
            <Link className="deliverable" href={item.href}>
              ...
              <span>{String(index + 1).padStart(2, "0")}</span><h3>{item.title}</h3><p>{item.note}</p>{"advanced" in item && item.advanced && <small className="deliverable__advanced">EXCLUSIVO ADVANCED · COSTO ADICIONAL</small>}<i>VER DETALLE ↗</i>
            </Link>
          </Reveal>
        ))}
      </div>
