...
function FinalCta() {
  return <section className="final-cta">
    ...
    <div className="button-row"><WhatsAppLink message={WHATSAPP_MESSAGES.general} eventId="whatsapp_final">Solicitar cotización</WhatsAppLink><a className="text-link" href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL} <Arrow /></a></div>
  </section>;
}
...
export function ServiceDetailPage({ id }: { id: PackageId }) {
  ...
  return <>
    ...
    <section className="section section--blue"><div className="container"><SectionHead eyebrow={`EJEMPLO ${id.toUpperCase()}`} title={example.title} /><div className="example-callout"><div><Image src={example.image} alt="" fill sizes="(max-width: 800px) 100vw, 55vw" /></div><div><p>{example.subtitle}</p><Link className="button button--primary" href={`/ejemplos/${id}`}>Abrir ejemplo <Arrow /></Link></div></div></div></section>
...
export function ExamplesPage() {
  return <>
    <section className="page-hero page-hero--center"><Breadcrumbs items={[["Inicio","/"],["Ejemplos"]]} /><span className="eyebrow">EJEMPLOS REALES</span><h1>Explora cada servicio<br /><em>por separado.</em></h1><p>Selecciona el nivel que quieres conocer. El recorrido interactivo se carga únicamente cuando lo abres.</p></section>
